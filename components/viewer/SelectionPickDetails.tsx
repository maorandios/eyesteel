...
import { Bolt, BookCheck, Workflow } from "lucide-react";
import type {
  AnalyzerAssembly,
  AnalyzerBoltRow,
...
function KeyValueList({
  ...
  return (
    <section className="mb-3">
      <h3 className="mb-1.5 flex items-center gap-1.5 px-1 text-xs font-semibold text-zinc-500">
        {icon}
        {title}
      </h3>
      <dl className="divide-y divide-zinc-200">
        {rows.map(({ label, value }) => (
          <div key={label} className="flex gap-3 px-2 py-2.5 text-xs leading-snug">
            <dt className="min-w-[8.5rem] shrink-0 text-zinc-500">{label}</dt>
            <dd className="flex-1 text-right font-medium text-zinc-900">{value}</dd>
          </div>
        ))}
      </dl>
    </section>
        ...
        {aggregatedBolts.length === 0 ? (
          ...
          <div className={`${PANEL_SCROLL} overflow-x-auto`}>
            <table className="w-full min-w-[20rem] text-xs">
              <thead className="text-[10px] text-zinc-500">
                <tr>
                  <th className="p-2 text-right font-medium">שם הבורג</th>
                  <th className="p-2 text-right font-medium">אורך (מ״מ)</th>
                  <th className="p-2 text-right font-medium">תקן</th>
                  <th className="p-2 text-right font-medium">קוטר חור (מ״מ)</th>
                  <th className="p-2 text-right font-medium">כמות</th>
                </tr>
              </thead>
              <tbody>
                {aggregatedBolts.map(({ key, bolt: b, effectiveQty }) => (
                  <tr key={key} className="border-t border-zinc-200">
                    ...
                    <td className="p-2 text-zinc-700">{b.boltStandard || EM_DASH}</td>
                    <td className="whitespace-nowrap p-2 text-zinc-700">
                      <span dir="ltr">{formatMmPlain(b.boltHoleDiameterMm)}</span>
                    </td>
                    <td className="p-2 text-zinc-700">{formatQuantityInt(effectiveQty)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="space-y-12" dir="rtl">
      ...
      <div className="pt-12">
        <KeyValueList
          title="נתונים טכניים"
          icon={<Workflow className="size-3.5 shrink-0" aria-hidden />}
          rows={technicalRows}
        />
      </div>
    </div>
